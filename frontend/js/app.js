...
function renderSummary(summaryPayload, meta) {
  ...
  const decodedSummary = decodeHTMLEntities(summaryPayload.content);
  state.summaryResult = decodedSummary;
  state.analysisResults.summary = decodedSummary;
  elements.summaryCard.classList.remove('hidden');
...
function updateDimensionContent(dimension, content, append = false) {
  ...
  if (append) {
    state.analysisResults[dimension] += decodedContent;
  } else {
    state.analysisResults[dimension] = decodedContent;
  }
